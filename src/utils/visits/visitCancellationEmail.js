...
module.exports.sendVisitCancellationEmail = async (
  userEmail,
  doctorEmail,
  visitDetails
) => {
  try {
      ...
      html: `
        <html>
          <body>
            ...
            <p>Your visit scheduled on ${formatDate(
              visitDetails.date
            )} has been cancelled.</p>
            <p>Other Details:</p>
            <p>Doctor: ${visitDetails.doctor.firstName} ${
        visitDetails.doctor.lastName
      }</p>
      ...
      html: `
        <html>
          <body>
            ...
            <p>Patient: ${visitDetails.patient.firstName} ${
        visitDetails.patient.lastName
      }</p>
            <p>Tax ID: ${visitDetails.patient.taxId}</p>
            <p>The visit scheduled on ${formatDate(
              visitDetails.date
            )} has been cancelled.</p>
          </body>
        </html>
    ...
    console.log(
      "Visit cancellation emails sent successfully:",
      infoUser.response,
      infoDoctor.response
    );
  } catch (err) {
