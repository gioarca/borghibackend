module.exports.validateLeaveRequests = (leaveRequests) => {
  for (const request of leaveRequests) {
    if (!request.typology) {
      return 'The "typology" field is required for each leave request.';
    }
    if (
      request.typology.toLowerCase() !== "vacation" &&
      request.typology.toLowerCase() !== "leaves"
    ) {
...
module.exports.checkDuplicateLeaveRequests = (
  existingRequests,
  newRequests
) => {
  ...
  for (const newRequest of newRequests) {
    const isDuplicate = existingRequests.some(
      (existingRequest) =>
        new Date(newRequest.startDate).getTime() ===
          new Date(existingRequest.startDate).getTime() &&
        new Date(newRequest.endDate).getTime() ===
