...
router.post(
  ...
  [
    ...
    check("workShifts")
      .optional()
      .isArray()
      .withMessage("Work shifts must be an array"),
    check("workShifts.*.dayOfWeek")
      .optional()
      .notEmpty()
      .isIn([
        "Monday",
...
router.post("/verify-email/:token", (req, res, next) =>
  verifyEmail(req, res, next, Doctor)
);
router.post("/verify-password", verifyToken, (req, res, next) =>
  verifyPassword(req, res, next, Doctor)
);
...
router.post("/generate2FA/:id", verifyToken, (req, res, next) =>
  generate2FA(req, res, next, Doctor)
);
